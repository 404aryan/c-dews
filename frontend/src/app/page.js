...
const CropCard = ({ name, imageUrl, description }) => (<div className="bg-white rounded-lg shadow-md overflow-hidden transform hover:-translate-y-2 transition-transform duration-300"><img src={imageUrl} alt={name} className="w-full h-40 object-cover"/><div className="p-4"><h3 className="text-lg font-semibold text-gray-800">{name}</h3><p className="text-sm text-gray-600 mt-1">{description}</p></div></div>);
const AdviceSection = ({ title, steps, icon }) => (<div className="mb-6"><h4 className="flex items-center text-xl font-bold text-gray-800 mb-3">{icon}{title}</h4><ul className="list-disc list-inside space-y-2 text-gray-700 leading-relaxed">{steps.map((step, index) => <li key={index} dangerouslySetInnerHTML={{ __html: step }}></li>)}</ul></div>);
...
export default function Home() {
  ...
  const handleLogout = () => { localStorage.removeItem('userToken'); setIsLoggedIn(false); window.location.reload(); };
  const supportedCrops = [{ name: 'Tomato', imageUrl: 'https://i.imgur.com/f70h5aE.jpeg', description: 'Frequently affected by blights and mosaic virus.' },{ name: 'Potato', imageUrl: 'https://i.imgur.com/nOy1g0r.jpeg', description: 'Susceptible to early and late blight diseases.' },{ name: 'Strawberry', imageUrl: 'https://i.imgur.com/k2n32y0.jpeg', description: 'Commonly suffers from leaf scorch and grey mold.' },{ name: 'Soybean', imageUrl: 'https://i.imgur.com/X42aJ9a.jpeg', description: 'Vulnerable to rust, downy mildew, and septoria.' },{ name: 'Pepper', imageUrl: 'https://i.imgur.com/0s6sS02.jpeg', description: 'Often sees bacterial spots and powdery mildew.' }];
    ...
    <div className="flex flex-col min-h-screen bg-gray-50 font-sans">
        ...
        <nav className="container mx-auto px-6 py-3 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-6">
            <div className="hidden md:flex items-center space-x-6 font-medium">
              <a href="/map" className="text-gray-600 hover:text-emerald-600 transition-colors">Outbreak Map</a>
            ...
            {isLoggedIn ? (
              <button onClick={handleLogout} className="flex items-center gap-2 px-4 py-2 text-red-600 font-semibold rounded-full hover:bg-red-50 transition-colors">
                <LogOut size={18} />
                Logout
              </button>
            ) : (
              <>
                <a href="/login" className="px-4 py-2 text-emerald-600 font-semibold rounded-full hover:bg-emerald-50 transition-colors">Login</a>
                <a href="/signup" className="px-4 py-2 text-white bg-emerald-600 rounded-full hover:bg-emerald-700 transition-colors font-semibold shadow-sm hover:shadow-md">Sign Up</a>
              </>
            )}
          </div>
        </nav>
      ...
      <main className="flex-grow">
        <section className="relative text-center py-24 md:py-40 text-white bg-cover bg-center" style={{ backgroundImage: "url('https://images.unsplash.com/photo-1492496913980-501348b61469?q=80&w=2787&auto=format&fit=crop')" }}>
          ...
          <div className="relative container mx-auto px-6">
            <h1 className="text-4xl md:text-6xl font-extrabold leading-tight drop-shadow-lg">Detect Crop Diseases in Seconds.</h1>
            <p className="mt-4 text-lg text-gray-200 max-w-2xl mx-auto drop-shadow-md">Snap a photo of a leaf — our AI identifies common plant diseases, gives clear confidence scores, and helps protect your harvest.</p>
          </div>
        </section>
        ...
        <section id="diagnose" className="py-20 bg-gray-50">
          <div className="w-full max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-lg border -mt-48 md:-mt-56 z-10 relative">
            ...
            <div className={`p-8 border-2 border-dashed rounded-lg text-center transition-colors duration-300 ${isDragOver ? 'border-emerald-500 bg-emerald-50' : 'border-gray-300'}`} onDragOver={(e) => handleDragEvents(e, true)} onDragLeave={(e) => handleDragEvents(e, false)} onDrop={handleDrop}>
              ...
              <label htmlFor="file-upload" className="cursor-pointer text-emerald-600 font-semibold hover:underline">{preview ? 'Change Image' : 'Choose an Image'}</label>
        ...
        <section id="about" className="py-20">
          <div className="container mx-auto px-6 text-center">
            ...
            <p className="max-w-3xl mx-auto text-lg text-gray-600 leading-relaxed">The Crop Disease Early Warning System (CDEWS) is a hackathon project designed to empower farmers with accessible technology. By leveraging machine learning, we provide instant, on-the-field diagnoses to help prevent crop loss, improve yields, and support sustainable agriculture. Our goal is to make agricultural knowledge universally available.</p>
          </div>
        </section>
        
        <section id="crops" className="py-20 bg-emerald-50/50">
          <div className="container mx-auto px-6">
            <SectionTitle>Initially Supported Crops</SectionTitle>
            ...
            <p className="mt-12 text-center text-gray-600">Our model is constantly learning. More crops and diseases will be added over time!</p>
          </div>
        </section>
        ...
        <section id="contact" className="py-20">
          <div className="container mx-auto px-6 text-center">
            ...
            <a href="mailto:contact@cdews-project.org" className="inline-flex items-center gap-2 px-8 py-3 text-white bg-emerald-600 rounded-full hover:bg-emerald-700 transition-colors duration-300 font-semibold shadow-sm hover:shadow-md">
              <Send size={20}/>
              <span>Email Us</span>
            </a>
          </div>
      ...
      <footer className="w-full bg-white border-t">
        <div className="container mx-auto px-6 py-6 text-center text-gray-600">
          <p>&copy; {new Date().getFullYear()} CDEWS. Built with <Heart className="inline-block text-red-500 h-4 w-4"/> for the Hackathon.</p>
        </div>
      </footer>
    </div>
  );
